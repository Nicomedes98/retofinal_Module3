...
export class InvoiceService {
    ...
    private invoices: InvoiceDto[] = [
        {
            customer: null,
            serie: 'A1',
            business_name: 'QueparecaunAchidenchi',
    ...
    async createInvoice(invoice: InvoiceDto, uuid: string): Promise<InvoiceDto> {
        ...
        if (customerobtained) {
            invoice.customer = customerobtained as CustomerDto
    ...
    patchInvoice(serie: string, invoice: PatchDto): PatchDto {
        const oldinvoice = this.invoices.find(invoice => invoice.serie === serie);
        if (oldinvoice) {
            let invoicepatched: PatchDto = {
                ...oldinvoice,
                ...invoice
            }
            this.invoices = this.invoices.map((invoice: PatchDto) => invoice.serie === serie ? invoicepatched : invoice)
